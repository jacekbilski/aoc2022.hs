...
day05_1 input = do
  ...
  load_stacks stacks_input |> apply_procedure procedure_input |> Map.map head |> Map.elems
...
apply_procedure :: [String] -> Stacks -> Stacks
apply_procedure input stacks = do
  let procedure = parse_procedure input
  do_apply_procedure procedure stacks

parse_procedure :: [String] -> [Step]
parse_procedure input = input |> map words |> map (\a -> (read (a !! 1), a !! 3 !! 0, a !! 5 !! 0))

do_apply_procedure :: [Step] -> Stacks -> Stacks
do_apply_procedure [] stacks = stacks
do_apply_procedure steps stacks = do
  let step = head steps
  let (quantity, source, target) = step
  if quantity == 0
    then do
      do_apply_procedure (tail steps) stacks
    else do
      let e = stacks Map.! source |> head
      Map.adjust ([e] ++) target stacks |> Map.adjust (tail) source |> do_apply_procedure ((quantity - 1, source, target) : (tail steps))
